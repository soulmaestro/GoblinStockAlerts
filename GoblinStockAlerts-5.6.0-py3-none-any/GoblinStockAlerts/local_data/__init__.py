...
from .. import project_pypi
from ..errors import GSAException, GSAOldDatabase
...
def load_db(*,
    ...
    # 90 days feels very fair, which is four times per year.
    last_updated = datetime.fromtimestamp(data.get('last_updated', 0))
    if (datetime.utcnow() - last_updated).days > 90:
        raise GSAOldDatabase(f"GSA database '{db}' is over 90 days old. It is highly likely it will be outdated. "
                             f"Please download the latest version of GSA from {project_pypi}.")
