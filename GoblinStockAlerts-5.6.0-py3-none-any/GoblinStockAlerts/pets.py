...
def check_pet_validity(*,
    ...
    :param nickname: The user specified nickname of the pet in question.
    :param pet: The pet dictionary object.
    :param scope: The scope the pet was found under, eg. a realm or "global".
    """
    ...
    if 'species_id' not in pet:
        raise GSAConfigurationError(f"{nickname} under {scope} is missing an species_id.")

    if not isinstance(pet['species_id'], int):
        raise GSAConfigurationError(f"{nickname} under {scope} is missing a valid species_id (no decimals).")
