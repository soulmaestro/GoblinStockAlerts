...
def convert_configuration_to_shopping_data(*,
                                           configuration: Dict
                                           ) -> Dict:
    """
    Convert a GSA .yml figuration to a dictionary configuration with all the necessary metadata needed by
    GoblinStockAlerts.
...
def create_realm_shopping_list(*,
    ...
    if item_config:
        for item in conf.get('items', {}):
            ...
            check_item_validity(nickname=item, item=data, scope=connected_realm)
    ...
    if pet_config:
        for pet in pet_config:
            ...
            check_pet_validity(nickname=pet, pet=data, scope=connected_realm)
...
def validate_configuration(*,
    ...
    if configuration['configuration']['version'] != 3.0:
    ...
    if 'connected_realm_nicknames' in configuration['configuration']:
        ...
        for nickname in nicknames:
            if not isinstance(nickname, int):
                raise GSAConfigurationError("The nickname must be attached to a connected realm id. This is a number.")

            if not nicknames[nickname] or not isinstance(nicknames[nickname], str):
                raise GSAConfigurationError(f"You need to actually specify a nickname for {nickname} if you're going"
                                            " to list it.")
