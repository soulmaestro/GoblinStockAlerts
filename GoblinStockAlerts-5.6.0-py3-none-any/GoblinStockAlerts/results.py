...
from .addon import reset_addon_data_file
    ...
    while True:
        try:
                ...
                if rs.states[connected_realm].status == STATE_SCHEDULED:
                    ...
                    if rs.states[connected_realm].response and rs.states[connected_realm].response.done():
                        ...
                        # Did it update?
                            ...
                            try:
                                deal_callback(configuration=configuration,
                                ...
                                logger.error(f"Processing deals encountered an exception: {ex}.")
            ...
            if updated == 0 and (datetime.utcnow() - timer).total_seconds() >= 300:
                logger.info(f"Waiting for Blizzard to update Auction House data...")
                timer = datetime.utcnow()
